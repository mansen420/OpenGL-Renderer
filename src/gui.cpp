...
//TODO make these flags for the file loading function
static bool load_obj_path          = false;
static bool load_shader_path       = false;
static const char* shader_filename = nullptr;
...
void workspace_panel()
{
    ...
    if (BeginTabBar("Workspace")) 
    {
                    ...
                    SeparatorText("Scene Parameters");
                    {
                        ...
                        {                
                            ...
                            case 0:
                                ...
                                load_and_update("john_the_baptist.fs");
                                break;
                            case 1:
                                ENGINE_SETTINGS.RENDER_W = ENGINE_SETTINGS.RENDER_H = 1000;
                                load_and_update("armored_guy.fs");
                                break;
                            case 2:
                                ENGINE_SETTINGS.LIGHT_POS = glm::vec3(0.0);
                                ENGINE_SETTINGS.RENDER_W = ENGINE_SETTINGS.RENDER_H = 300;
                                load_and_update("bunny.fs");
                                break;
                            case 3:
                                ENGINE_SETTINGS.RENDER_W = ENGINE_SETTINGS.RENDER_H = 300;
                                load_and_update("2D.fs");
                                break;
                            case 4:
                                ENGINE_SETTINGS.RENDER_W = ENGINE_SETTINGS.RENDER_H = 1000;
                                load_and_update("3D.fs");
                                break;
                        case 5:
                                ENGINE_SETTINGS.RENDER_W = ENGINE_SETTINGS.RENDER_H = 1000;
                                load_and_update("3D_shiny.fs");
                                break;
                            default:
                                break;
                            }
        ...
        if(BeginTabItem("Shaders"))
        {
            ...
            static bool keep_file_updated = false;
            SameLine();
            Checkbox("Keep Updated", &keep_file_updated);
            ...
            {
                load_and_update(shader_filename, prog_type, shader_type);
                shader_filename = nullptr;
            }
            PushItemWidth(GetWindowWidth()*0.25);
...
void conditional_gui()
{
    if (should_show_filedialog)
    {
        ...
        if (window::file_dialog.HasSelected())
        {
            ...
            {
                //FIXME this is just dirty
                static std::string file_path = window::file_dialog.GetSelected().c_str();
                shader_path = file_path.c_str();
                static std::string filename;
                filename = std::string(window::file_dialog.GetSelected().filename());
                shader_filename = filename.c_str();
            }
