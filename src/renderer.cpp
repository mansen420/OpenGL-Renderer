...
static std::ofstream eng_log;
       ...
       engine_state_t  engine::ENGINE_SETTINGS;    //public state 
...
const char* engine::get_shader_source_reflection(shader_prg_option program_type, shader_type_option shader_type)
{  
...
std::string engine::get_shader_source_copy(shader_prg_option program_type, shader_type_option shader_type)
{
    const shader_manager::shader_t* shader = find_shader(program_type, shader_type);
    return shader->source_code;
}
std::string* engine::get_shader_source_reference(shader_prg_option program_type, shader_type_option shader_type)
{
...
bool engine::update_shader(shader_prg_option program_type, shader_type_option shader_type, const char* source)
{
    shader_manager::shader_t* shader = find_shader(program_type, shader_type);
...
bool engine::unroll_includes(shader_prg_option program_type, shader_type_option shader_type)
{
    shader_manager::shader_t* shader = find_shader(program_type, shader_type);
    if (nullptr == shader)
    {
...
bool engine::link_program(shader_prg_option program_type)
{
...
bool engine::load_source(const char* filename, char* &source_holder)
{   
...
size_t engine::object_nr_vertices()
{
    return obj_ptr->nr_vertices();
}
size_t engine::object_nr_triangles()
{
...
void send_shadow_map_uniforms()
{
    ...
    glm::mat4 light_perspective_transform;
    if(internal_state.SHADOW_MAP_PROJECTION == ORTHOGRAPHIC)
        light_perspective_transform = glm::ortho(-10.f, 10.f, -10.f, 10.f, 0.1f, 10.f);
    else 
        light_perspective_transform = glm::perspective(glm::radians(45.f), 1.f, 0.5f, 5.f);
...
void postprocess_pass()
{
    ...
    else if(internal_state.DISPLAY_BUFFER != COLOR)
    {
        ...
        if(internal_state.DISPLAY_BUFFER == SHADOW_MAP)
            glBindTexture(GL_TEXTURE_2D, shadowmap_depth_tex_ID);
...
void engine::update_offscreen_tex_params()
{
    glBindTexture(GL_TEXTURE_2D, offscr_tex_IDs[COLOR_TEX_IDX]);
...
void engine::update_screen_tex_coords()
{   
