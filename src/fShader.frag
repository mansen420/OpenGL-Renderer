...
void main()
{
    ...
    fragment_output = vec4(shade_spot(test_light), 1);
}
...
vec3 shade_spot(spotlight s_light)
{
    ...
    if (cos_phi>=s_light.cosine_angle)
    {
        ...
        float attenuation_factor = (cos_phi-s_light.cosine_angle);  //[0, 1-cosigne_angle]
        attenuation_factor /= (1-s_light.cosine_angle); //[0, 1]
        return distance_attenuation*(1*attenuation_factor + 1.2*pow(attenuation_factor, 2))*   //sharpness factor * attenuation factor 
